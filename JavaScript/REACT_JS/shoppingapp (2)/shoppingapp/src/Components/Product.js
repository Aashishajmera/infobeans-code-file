...
// import viewProductDetails from "./viewProduct";
...
export default (props)=>{
  ...
  const addToCart = (productId)=>{
    axios.post("http://localhost:3000/cart/addToCart",{userId: sessionStorage.getItem("user_id"), productId})
    .then(response=>{
  ...
  const viewProductHandler = (product)=>{
    navigate("/viewProductDetails",{state:product})
}      
        ...
        <div className="row">
          {productList.map((product,index)=><div className="col-md-4 p-3" key={index}>
            <div className="d-flex flex-column align-items-center" style={{height:"400px", boxShadow: '0 0 5px grey'}}>
                ...
                <small className="text-primary" onClick={()=>viewProductHandler(product)} style={{cursor:"pointer"}}>View more</small>
