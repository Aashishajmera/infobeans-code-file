...
public class B11_Account implements ActionListener {
    ...
    public void createaccount() {
        try {
            fram5.setTitle("BANKING_APPLICATION");
    ...
    public void actionPerformed(ActionEvent action5) {
        try {
            ...
            if (action5.getSource() == submit) {
                if (!fname.matches("^[a-zA-Z]+$") || !(fname.length() >= 2) || !(fname.length() <= 20) ||fname.equals(null)||fname.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid First Name");
                } else if (!lname.matches("^[a-zA-Z]+$") || !(lname.length() >= 2) || !(lname.length() <= 20)||lname.equals(null)||lname.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid last Name");
                } else if (!father.matches("^[a-z A-Z.]+$") || !(father.length() >= 2) || !(father.length() <= 50)||father.equals(null)||father.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid Father Name");
                } else if (!mobile.matches("^[0-9]+$") || !(mobile.length() == 10)||mobile.equals(null)||mobile.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid Mobile Number");
                } else if (!address.matches("^[1-9a-zA-Z(),.-_/ ]+$") || !(address.length() >= 2)||address.equals(null)||address.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid Address");
                } else if (!date.matches("^[0-9-]+$") || !(date.length() == 10)||date.equals(null)||date.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid Date Of Birth");
                } else if (!email.matches("^[a-zA-Z0-9@.]+$") || !(email.length() >= 13) || !(email.length() <= 30)||email.equals(null)||email.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid E-Mail");
                } else if (!aadhar.matches("^[0-9]+$") || !(aadhar.length() == 12)||aadhar.equals(null)||aadhar.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid Aadhar Number");
                } else if (!pan.matches("^[0-9A-Z]+$") || !(pan.length() == 10)||pan.equals(null)||pan.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid Pan Number");
                } else if (!nominee.matches("^[a-z A-Z.]+$") || !(nominee.length() >= 2) || !(nominee.length() <= 50)||nominee.equals(null)||nominee.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid Nominee Name");
                } else if (gender.equals("")||gender.equals(null)||gender.equals("")) {
                    JOptionPane.showMessageDialog(null, "Enter Gender");
                } else if (!amount.matches("^[0-9]+$")||amount.equals(null)||amount.equals("")) {
                    JOptionPane.showMessageDialog(null, "Invalid Amount");
                } else if ((am < 1000)) {
                    JOptionPane.showMessageDialog(null, "Please Enter atleast 1000 Rs");
                } else {
                    try {
                        D_DatabaseConnection obj = new D_DatabaseConnection();
                        String q1 = "insert into account values (null,'" + fname + "','" + lname + "','" + father + "','" + mobile + "','" + address + "','" + gender + "','" + email + "','" + aadhar + "','" + pan + "','" + account + "','" + branch + "'," + amount + ",'" + nominee + "','" + password + "','" + date + "',default)";
                        obj.s.executeUpdate(q1);
                        JOptionPane.showMessageDialog(fram5, "Account Created Successfully", "Alert", JOptionPane.WARNING_MESSAGE);
                        fram5.setVisible(false);
                        fram5.dispose();
                    } catch (Exception e) {
                        JOptionPane.showMessageDialog(fram5, "Please Fill the Form", "Alert",JOptionPane.ERROR_MESSAGE);
                    }
            ...
            // JOptionPane.showMessageDialog(fram5, "Somthing Went Wrong", "Alert",
