...
export const viewParticularProducts = async (request, response, next) => {
    const errors = validationResult(request);
    if (!errors.isEmpty())
        return response.status(401).json({ error: errors.array() });

    Product.findOne({ where: { id: request.body.id } })
        .then(result => {
            if (result)
                return response.status(200).json({ productList: result });
            return response.status(401).json({ error: "unautharized request", });
        }).catch(err => {
            return response.status(500).json({ error: "Internal Server Error", err });
        })
}
