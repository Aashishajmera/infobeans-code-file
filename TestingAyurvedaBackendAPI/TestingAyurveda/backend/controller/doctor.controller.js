...
export const doctorFeedback = (req, res, next) => {
  const error = validationResult(req);
  if (!error.isEmpty()) return res.status(401).json({ error: error.array() });

  let ratingStar = req.body.ratingStar;
...
export const signIn = async (request, response, next) => {
  ...
  if (doctor) {
    if (Doctor.checkPassword(password, doctor.password)) {
      ...
      return response
        .status(200)
        .json({ message: "Sign In Success", doctor, token: token });
    } else {
      return response.status(401).json({ error: "Unauthorized doctor" });
    }
  } else return response.status(401).json({ error: "Unauthorized doctor" });
};
...
export const SignUp = (request, response, next) => {
  ...
  Doctor.create({
    ...
    .then((result) => {
      return response
        .status(200)
        .json({ data: result.dataValues, message: "Doctor created..." });
    })
    .catch((err) => {
      console.log(err);
      return response
...
export const appointment = (request, response, next) => {
  const errors = validationResult(request);
  if (!errors.isEmpty())
    return response.status(401).json({ error: errors.array() });
  ...
  DoctorDetail.update(
    ...
    .then((result) => {
      if (result[0])
        return response
          .status(200)
          .json({ message: "DoctorDetail updated...." });
      return response.status(401).json({ message: "unauthorized request...." });
    })
    .catch((err) => {
      return response
...
export const doctordata = (request, response, next) => {
  Doctor.findAll({ include: [{ model: DoctorDetail, required: true }] })
    .then((result) => {
      return response.status(200).json({ data: result });
    })
    .catch((err) => {
...
export const doctordatabyemail = (request, response, next) => {
  const errors = validationResult(request);
  if (!errors.isEmpty())
    return response.status(401).json({ error: errors.array() });
